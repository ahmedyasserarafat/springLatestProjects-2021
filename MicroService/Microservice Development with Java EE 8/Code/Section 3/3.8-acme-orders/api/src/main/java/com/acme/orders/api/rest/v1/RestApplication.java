...
import com.acme.orders.api.rest.v1.auth.OrdersAuthenticator;
import com.acme.orders.api.rest.v1.auth.OrdersAuthorizer;
...
public class RestApplication extends Application<RestConfiguration> {
    ...
    public void run(RestConfiguration configuration, Environment environment) throws Exception {
        ...
        Client integrationClient = ClientBuilder.newClient().register(JacksonProvider.class);

        OrderService orderService = new OrderServiceImpl(
                new OrderDAO(hibernate.getSessionFactory(), environment.metrics()),
                new CustomersClientImpl(integrationClient, configuration.getCustomersUrl(), environment.metrics()),
                new CatalogueClientImpl(configuration.getCatalogueUrl()),
                new PaymentsClientImpl(integrationClient, configuration.getPaymentsUrl()),
                environment.metrics());

        environment.jersey().register(new OrderResource(orderService));

        environment.jersey().register(EmptyPayloadMapper.class);
//        environment.jersey().register(FormatNotAcceptableMapper.class);
//        environment.jersey().register(FormatNotSupportedMapper.class);
        ...
        environment.jersey().register(new AuthDynamicFeature(
                new OAuthCredentialAuthFilter.Builder<User>()
                        .setAuthenticator(new OrdersAuthenticator(getAuthAlgorithm(configuration.getAuthPublicKey())))
                        .setAuthorizer(new OrdersAuthorizer())
                        .setPrefix("Bearer")
                        .buildAuthFilter()));
        ...
        environment.healthChecks().register("customers-api", new IntegrationHealthCheck(healthClient, configuration.getCustomersUrl()));
        environment.healthChecks().register("payments-api", new IntegrationHealthCheck(healthClient, configuration.getPaymentsUrl()));
    }
